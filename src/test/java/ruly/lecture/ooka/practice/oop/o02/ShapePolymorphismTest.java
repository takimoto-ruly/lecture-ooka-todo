...
class ShapePolymorphismTest {
	...
	void testIndividualBehavior() {
		Shapes genericShape = new Shapes();
		Circle circle = new Circle();
		Square square = new Square();

		assertEquals("図形を描画します", genericShape.drawing());
		assertEquals("円を描画します", circle.drawing(), "Circleクラスで正しくオーバーライドされている必要があります");
		assertEquals("四角を描画します", square.drawing(), "Squareクラスで正しくオーバーライドされている必要があります");
	}
	...
	void testPolymorphicBehavior() {
		...
		Shapes polymorphicShape = new Circle();

		assertEquals("円を描画します", polymorphicShape.drawing(),
